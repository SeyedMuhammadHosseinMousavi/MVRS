...
# === Load data ===
print("Loading data...")
df = pd.read_csv("Extracted_GSR_EMG_AllFeatures.csv")
...
# === Features and target ===
X = df.drop(columns=['emotion', 'participant'], errors='ignore')
y = df['emotion'].values
...
def per_class_conf_counts(y_true, y_pred, cls):
    ...
    # Confusion matrix for binary case -> [[TN, FP], [FN, TP]]
...
def per_class_metrics_ovr(y_true, y_pred, n_classes):
    # Returns dict keyed by class index with:
    # precision, recall, f1, acc_ovr, kappa_ovr, mcc_ovr, uar_class (= recall)
    ...
    for c in range(n_classes):
        ...
        # po = (TP+TN)/N; pe = ((TP+FP)/N)*((TP+FN)/N) + ((TN+FN)/N)*((TN+FP)/N)
        po = safe_div(TP + TN, N)
...
for model_name, model in models.items():
    ...
    # === Final aggregated results (keep existing detailed prints) ===
    ...
    # === Print the seven FINAL metrics per class ===
    ...
    # === Print the seven FINAL metrics for all classes ===
    ...
    # print(f"  UAR (Macro Recall): {uar_macro:.4f}")

    # Also echo mean fold accuracy for reference
