...
# === Load CSV ===
print("Loading data...")
df = pd.read_csv("Fused_Early_Selected.csv")

# === Separate features and labels ===
X = df.drop(columns=["emotion", "participant"], errors="ignore").values
y_raw = df["emotion"].values

# === Encode labels (always) to ensure 0..K-1 labeling and names ===
...
# === Global mean imputation (keep consistent with your template) ===
print("Imputing missing values...")
imputer = SimpleImputer(strategy="mean")
X = imputer.fit_transform(X)

# === Define classifiers (parallel where applicable) ===
models = {
...
# === Train and evaluate each classifier with scaling+PCA inside each fold ===
for name, model in models.items():
    ...
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X), start=1):
        ...
        # PCA (fit on train only) — cap at 60 and <= min(n_samples_train, n_features)
        n_components = int(min(60, X_train_scaled.shape[1], X_train_scaled.shape[0]))
        if n_components < 1:  # safety
            n_components = min(1, X_train_scaled.shape[1])
